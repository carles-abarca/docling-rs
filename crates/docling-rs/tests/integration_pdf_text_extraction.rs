...
fn test_extract_text_from_empty_pdf() {
    ...
    // Empty PDF should have no paragraph nodes (may have page headings)
    let has_paragraph_content = doc.nodes().iter().any(|node| {
        node.node_type() == docling_rs::NodeType::Paragraph
            && node
                .text_content()
    ...
    assert!(
        ...
        "Empty PDF should have no paragraph content in nodes"
    );
}
